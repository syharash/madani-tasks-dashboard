...
onMounted(() => {
  SocialLogin.initialize({
    google: {
      webClientId: '515935803707-v7qshp425m1b4h5ru6jcmmmu99qbikgq.apps.googleusercontent.com',
    }
  })
})
...
function setupSignIn() {
  ...
  signinBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      document.getElementById("statusBanner").textContent = `Welcome, ${res.name || res.email || "User"}`;
    } catch (err) {
      console.error("❌ Login failed:", err);
      document.getElementById("statusBanner").textContent = "Login failed. Please try again.";
    }
  });
}
...
async function fetchRegionContext(config) {
  const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${config.id}/values/Sheet1!A1:A3`, {
    headers: { Authorization: `Bearer ${accessToken}` }
  });
  const data = await res.json();
